    ...
    <section class="products">
      ...
      <div class="card-container">
        <div v-for="(product, index) in popularProducts" :key="'popular-' + index" class="card">
          ...
          <div class="add-btn">
            <button v-if="product.qty > 0" class="minus-icon" @click="removeFromCart(product)">
              -
            </button>
            <button
              class="add-text"
              :class="{ 'no-left-radius': product.qty > 0 }"
    ...
    <div class="cart-sidebar" :class="{ active: cart.length > 0 }">
      ...
      <div class="cart-items">
        ...
        <div v-for="(item, index) in cart" :key="'cart-item-' + index" class="cart-item">
          ...
          <div class="item-total">{{ (item.price * item.boughtQty).toLocaleString() }} MMK</div>
        </div>
      </div>
...
const checkout = () => {
  alert(
    `လူကြီးမင်း ဝယ်ယူမှု အောင်မြင်ပါသည်ရှင်။ စုစုပေါင်းကျသင့်ငွေ ${totalCartPrice.value.toLocaleString()} MMK ဖြစ်ပါတယ်ရှင်။`,
  )
  clearCart()
}
</script>
