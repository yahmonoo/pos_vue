        ...
        <tbody>
          ...
          >
            ...
            <td class="text-center">
              <v-btn density="compact" icon="mdi-pencil" @click="editTownship(item)"></v-btn>
              <v-btn density="compact" icon="mdi-delete" @click="deleteTownship(item)"></v-btn>
            </td>
          </tr>
        </tbody>
      </v-table>
      ...
      <v-card rounded="xl" class="cdialog">
        ...
        <div class="dialog-header">
          <div class="d-flex align-center">
            <div>
              <div class="text-h6 font-weight-bold">Add New Township</div>
            </div>
          </div>
        </div>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn rounded="pill" class="add-btn" @click="saveTownship"> {{ saveOrUpdate }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-dialog v-model="dialogDelete" width="500">
        <v-card>
          ...
          <v-card-text class="text-h6">
            Are you sure to delete({{ selectedOne.townshipName }})?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    cityListMethod() {
      cityService
        ...
        .then((response) => {
          ...
          this.townshipDto.cityDto = this.cityList[0]
        })
        .catch((error) => {
    ...
    saveTownship() {
      if (this.saveOrUpdate == 'SAVE') {
        console.log(this.saveOrUpdate)
                    console.log(this.townshipDto);


        townshipService
          .addTownship(this.townshipDto)
          .then((response) => {
            console.log(response);
            this.dialog = false;
            this.townshipDto = {};
    ...
    deleteTownship(item) {
      ...
      console.log(item)
      console.log(townshipDto);
    },
    clickDeleteDialog() {
      townshipService
        .deleteTownship(this.selectedOne)
        .then((response) => {
          console.log(response);
